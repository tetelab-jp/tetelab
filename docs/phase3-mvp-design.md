...
ライブサイトのDOM検証が必要な部分（画像アップロード等）と、検証不要でDB/画面だけで
進められる部分を分離し、後者を先に進められるようにした。

| Phase | 内容 | ライブサイト検証要否 |
|---|---|---|
| 3-A | `migrations/0004`でデータモデル移行（3章） | 不要 |
| 3-B | 接続設定画面の拡張（同期状態・エラー表示） | 一部要（ログイン確認のみ） |
| 3-C | スタイル一覧・作成/編集画面の再設計（担当スタイリスト紐付け、状態列表示） | 不要 |
| 3-D | テンプレート管理の複数化＋個別適用・一括適用モーダル | 不要 |
| 3-E | 実行履歴画面の汎用化（3状態表示、再実行ボタン） | 不要 |
| 3-F | スタイリスト/クーポン同期（`syncStylists`/`syncCoupons`） | 要（実際のマスタページDOM確認） |
| 3-G | 既存スタイル取り込み（`fetchExistingStyles`/`fetchStyleDetail`） | 要 |
| 3-H | 投稿実行フローへの状態遷移組み込み・NG/未確認判定 | 要 |
| 3-I | 写真アップロードの実装検証（HANDOFF.md記載の最大の難所） | 要（最優先で実サイト確認） |

3-A〜3-Eはテスト用SALON BOARDアカウントが無くても進められるため、先行着手を推奨する。
