...
**訂正（2026-08-08）**: 当初この表はスクリーンショット（競合ツール「スタイルポスト」の画面）から
採取した数値を誤ってSALON BOARD実測値として記載していた。同日、実際のSALON BOARDの
HTMLソース（`/CNB/draft/styleEdit/`の`initLength()`内`setCounterEvent()`呼び出し）を
確認し、正しい値に訂正した。

| フィールド | 上限 | 出典 |
|---|---|---|
| スタイル名(`styleNameTxt`) | **60文字** | 実HTML: `setCounterEvent('styleNameTxt', 'txtCntStyle', 60, ...)` |
| スタイリストコメント(`stylistCommentTxt`) | **240文字** | 実HTML: `setCounterEvent('stylistCommentTxt', ..., 240, ...)` |
| メニュー内容/フリーテキスト(`menuDetailTxt`) | **100文字** | 実HTML: `setCounterEvent('menuDetailTxt', ..., 100, ...)` |
| ハッシュタグ（1個あたり） | 40文字 | 実HTML: `<input ... maxlength="40" id="hashTagTxt" />` |
| ハッシュタグ（個数上限） | 未確認 | `hashTag.js`（外部ファイル、今回のHTMLに含まれず）に依存。実際に追加してみないと不明 |

### 4-6. SALON BOARD上のマスタ値（実HTMLで確定）

- **長さ**: レディース `frmStyleEditStyleDto.ladiesHairLengthCd` — `HL01`=ロング, `HL02`=セミロング,
  `HL03`=ミディアム, `HL04`=ショート, `HL05`=ベリーショート, `HL07`=ミセス, `HL08`=ヘアセット。
  メンズ `frmStyleEditStyleDto.mensHairLengthCd` — `HL09`=ボウズ, `HL10`=ベリーショート,
  `HL11`=ショート, `HL12`=ミディアム, `HL13`=ロング, `HL06`=その他
- **メニュー内容**: `MC01`=パーマ, `MC02`=ストレートパーマ・縮毛矯正, `MC03`=エクステ, `MC04`=ブリーチ
- **モデル属性**（`frmStyleEditStyleModelDto.*`）:
  - 髪量/髪質/太さ/クセ共通: `99`=設定しない, `1`〜`3`(少ない〜多い 等)
  - 顔型(`modelFaceTypeKbn`): `99`=設定しない, `4`=逆三角, `1`=丸型, `5`=ベース, `2`=卵型, `6`=面長, `3`=四角
  - 年代(`modelAgeKbn`): `99`=設定しない, `0`=キッズ, `1`=10代, `2`=20代, `3`=30代, `4`=40代, `5`=50代, `6`=60代以上
- **スタイリストID形式**: `T001014365`のような`T`+9桁の形式（スタイリスト一覧・スタイル一覧の
  両方で一致することを実HTMLで確認済み）
- **クーポンID形式**: `CP00000010256490`のような`CP`+14桁の形式
- **画像ID形式**: `B231655711`のような`B`+9桁の形式

### 4-7. SALON BOARD側の実際の上限（実測・当初想定と異なる）

「スタイル掲載可能枚数」ポップアップ（スタイル一覧画面）の実測値。多くのプランで
**9999枚まで**登録可能（EPプランのみ4枚）。当初、競合ツールの自主制限（サロンパラダイス36枚、
スタイルポスト100枚）をSALON BOARD自体の上限と誤認していたが、**これらは各ツール側が
独自に設けていた上限であり、SALON BOARD自体の上限ではない**。Pick Up枠のみSALON BOARD側の
実制限として「最大3つまで」と明記されている。

この誤認を踏まえ、7章「MVPに入れない機能」および6章の一括処理「100件まで」は、
SALON BOARD側の制約ではなく、**あくまでTETE AOUT側の運用上の初期上限（システム負荷・
UI設計上の判断）として設定する**ものと位置づけを修正する。
...
| Phase | 内容 | ライブサイト検証要否 | 状況(2026-08-08) |
|---|---|---|---|
...
## 9. 未確定・要確認事項（2026-08-08 実HTML入手により更新）

ユーザーから実際のSALON BOARD（ログイン画面／スタイル一覧／スタイル編集／
スタイリスト一覧／クーポン一覧）のHTMLソース一式を入手し、以下が解消した。

### 解消した事項
1. ~~画像アップロードの実際の内部動作~~ → **モーダル形式であることを確定**。
   `.img_new_no_photo`クリックで`img_upload_modal_view('FRONT_IMG_ID', 'ABNKD3600_FRONT', dataKey, false, 'styleEditForm')`
   が発火し、`#imageUploaderModalBody`にモーダル内容が動的挿入される。
   アップロード完了コールバックは`setUploadImage(imageId, setImgId, meetStandardFlg, lengthSize, sideSize, resolution, imageFilePath)`
   で、`#FRONT_IMG_ID`等の隠しフィールドに画像IDをセットする。
   **ただし**モーダル内部のDOM（実際の`<input type=file>`等）はJSで動的挿入されるため、
   静的HTMLソースには含まれていない。実際にクリックして初めて見える。9-1（下記）参照。
2. ~~スタイル一覧・編集画面の実際のHTML~~ → 入手済み。フォームid/name属性、
   `doRegister()`/`editStyle()`/`addStyle()`等のJS関数の実装を確認済み。
3. ~~クーポン選択のUI~~ → モーダル形式(`.jsc_SB_modal_trigger.jsc_SB_modal_single_coupon`)
   であることを確定。ただし最終的にはPOST時に隠しフィールド`frmStyleEditStyleDto.couponId`に
   クーポンID(`CP00000010256490`形式)が入っていればよいだけなので、**モーダルUIを操作せず
   `page.evaluate()`で直接この隠しフィールドに値をセットする方式で自動化できる可能性が高い**
   （要・実地での動作確認）。
4. ~~スタイリスト一覧／クーポン一覧ページのHTML~~ → 入手済み。`syncStylists()`/`syncCoupons()`
   の実装に着手可能（10章参照）。

### まだ残っている事項
5. **画像アップロードモーダルの内部DOM**：4章の通り動的挿入のため、実際にテスト用
   アカウントでクリックし、モーダルが開いた状態のDOM（またはDevTools Networkタブでの
   アップロードAJAXリクエスト）を確認する必要がある。Phase 3-Hの最優先事項として残る。
6. **ハッシュタグの個数上限**：`hashTag.js`（外部ファイル、今回未取得）の実装依存。
7. **反映申請フロー（掲載管理TOP）の実HTML**：`reflected(event, element)`関数の実装は
   まだ実HTMLで確認できていない（HANDOFF.md記載の理解のみ）。NG/未確認の判定ロジック
   （`checkReflectBlockers()`）実装前に確認が望ましい。
8. **ブログ投稿画面の実HTML**：引き続き未取得（HANDOFF.md 4-7から変わらず）。
