-- ============================================
-- 0005_ranking.sql (PostgreSQL / 追加マイグレーション)
--
...
--
-- ※このファイルは正本(スキーマの記録)。実際の適用は、専用ランナーが無いため
--   アプリ起動時に src/index.tsx の冪等ブロックが CREATE TABLE IF NOT EXISTS で
--   自動適用する(0002〜0004と同じ運用)。手動でRDSへ流すことも可能。
-- ============================================
...
-- --------------------------------------------
CREATE TABLE IF NOT EXISTS salonboard_salons (
...
-- --------------------------------------------
CREATE TABLE IF NOT EXISTS ranking_areas (
  id SERIAL PRIMARY KEY,
...
CREATE TABLE IF NOT EXISTS ranking_query_keywords (
  id SERIAL PRIMARY KEY,
